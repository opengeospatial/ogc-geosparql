...
1. WKB made no allowance for more than an X and Y coordinate, such as the Z and M coordinates that are common in GIS applications.
2. WKB made no allowance for embedding a spatial reference identifier (SRID), which made it unusable as a database import/export format.
...
1. Simple Features, which is used for definitions of spatial features.
2. WKB, upon which it is based.
3. GeoSPARQL 1.3, which requires it as an encoding. 
...
URL: https://libgeos.org/specifications/wkb/#:~:text=The%20%E2%80%9CExtended%20WKB%E2%80%9D%20variant%20is%20a%20superset%20of,existing%20wkbType%20that%20appears%20in%20all%20WKB%20geometries.

== Supporting OGC Members

The Submitting OGC Members for this work item are:
1. Geoscape Australia
2. Oracle (Voting Member)
4. Kurrawong AI
5. Defence Science & Technology Labs (DSTL) (Voting Member)
