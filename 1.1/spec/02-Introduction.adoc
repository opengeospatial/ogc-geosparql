...
Subjects and objects within an RDF triple are called nodes and can also be represented with a blank node (a local identifier with meaning outside the graph it is defined within). Objects can further be represented with a literal value. Basic literal values in RDF are those used in XML <<XSD2>> but the basic types can be extended for specialized purposes. Indeed, this specification extends the basic types to include geometry data. The figure below shows a basic triple.
...
** templated within this specification document
