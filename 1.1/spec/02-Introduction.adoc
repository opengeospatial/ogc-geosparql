...
While it is possible to encode threedimensional data in literals, the functions provided by the GeoSPARQL standard only provide support for 2.5D data.
This means that one could use GeoSPARQL-defined literals to store and retrieve 3D data in literals, but will not at this point be able to process native 3D data in-query.
