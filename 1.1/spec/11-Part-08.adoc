...
|===
| *Req 14* All RDFS Literals of type <<RDFS Datatype: geo:wktLiteral, WKT Literal>> shall consist of an optional IRI identifying the coordinate reference system and a required Well Known Text (WKT) description of a geometric value. Valid <<RDFS Datatype: geo:wktLiteral, WKT Literal>> instances are formed by either a WKT string as defined in <<ISO13249>> or by concatenating a valid absolute IRI, as defined in <<IETF3987>>, enclose in angled brackets (`<` & `>`) followed by a single space (Unicode U+0020 character) as a separator, and a WKT string as defined in <<ISO13249>>.
...
|===
| *Req 15* The IRI http://www.opengis.net/def/crs/OGC/1.3/CRS84[`+<http://www.opengis.net/def/crs/OGC/1.3/CRS84>+`] shall be assumed as the spatial reference system for <<RDFS Datatype: geo:wktLiteral, WKT Literal>> instances that do not specify an explicit spatial reference system IRI.
...
|===
| *Req 16* Coordinate tuples within <<RDFS Datatype: geo:wktLiteral, WKT Literal>> shall be interpreted using the axis order defined in the spatial reference system used.
...
|===
| *Req 17* An empty RDFS Literal of type <<RDFS Datatype: geo:wktLiteral, WKT Literal>> shall be interpreted as an empty geometry.
...
|===
| *Req 18* Implementations shall allow the RDF property <<Property: geo:asWKT, as WKT>> to be used in SPARQL graph patterns.
...
|===
| *Req 20* All <<RDFS Datatype: geo:gmlLiteral, GML Literal>> instances shall consist of a valid element from the GML schema that implements a subtype of `GM_Object` as defined in <<OGC07-036>>.
...
|===
| *Req 21* An empty <<RDFS Datatype: geo:gmlLiteral, GML Literal>> shall be interpreted as an empty geometry.
...
|===
| *Req 23* Implementations shall allow the RDF property <<Property: geo:asGML, as GML>> to be used in SPARQL graph patterns.
...
|===
| *Req 25* All <<RDFS Datatype: geo:geoJSONLiteral, GeoJSON Literal>> instances shall consist of the Geometry objects as defined in the GeoJSON specification <<GEOJSON>>.
...
|===
| *Req 27* An empty RDFS Literal of type <<RDFS Datatype: geo:geoJSONLiteral, GeoJSON Literal>> shall be interpreted as an empty geometry, i.e. `{"geometry": null}` in GeoJSON .
...
|===
| *Req 28* Implementations shall allow the RDF property <<Property: geo:asGeoJSON, as GeoJSON>> to be used in SPARQL graph patterns.
...
|===
| *Req 30* All <<RDFS Datatype: geo:kmlLiteral, KML Literal>> instances shall consist of the Geometry objects as defined in the KML specification <<OGCKML>>.
...
|===
| *Req 31* RDFS Literals of type <<RDFS Datatype: geo:kmlLiteral, KML Literal>> do not contain a SRS definition. All literals of this type shall according to the KML specification only be encoded in and assumed to use the WGS84 geodetic longitude-latitude spatial reference system (http://www.opengis.net/def/crs/OGC/1.3/CRS84[`http://www.opengis.net/def/crs/OGC/1.3/CRS84`]).
...
|===
| *Req 32* An empty RDFS Literal of type <<RDFS Datatype: geo:kmlLiteral, KML Literal>> shall be interpreted as an empty geometry .
...
|===
| *Req 33* Implementations shall allow the RDF property <<Property: geo:asKML, as KML>> to be used in SPARQL graph patterns.
...
|===
| *Req 35* All RDFS Literals of type <<RDFS Datatype: geo:dggsLiteral, DGGS Literal>> shall consist of a DGGS geometry serialization formulated according to a specific DGGS.
...
|===
| *Req 36* An empty RDFS Literal of type <<RDFS Datatype: geo:dggsLiteral, DGGS Literal>>, or one of its data subtypes, shall be interpreted as an empty `geo:Geometry`.
...
|===
| *Req 40* Implementations shall support the functions <<Function: geof:distance, distance>>, <<Function: geof:buffer, buffer>>, <<Function: geof:intersection, intersection>>, <<Function: geof:union, union>>, 
...
|===
| *Req 42* Implementations shall support http://www.opengis.net/def/function/geosparql/minX[`geof:maxX`],
http://www.opengis.net/def/function/geosparql/maxY[`geof:maxY`], http://www.opengis.net/def/function/geosparql/maxZ[`geof:maxZ`],  http://www.opengis.net/def/function/geosparql/minX[`geof:minX`], http://www.opengis.net/def/function/geosparql/minY[`geof:minY`], http://www.opengis.net/def/function/geosparql/minZ[`geof:minZ`], 
http://www.opengis.net/def/function/geosparql/boundingCircle[`geof:boundingBox`], 
http://www.opengis.net/def/function/geosparql/boundingCircle[`geof:boundingCircle`], 
http://www.opengis.net/def/function/geosparql/concatLines[`geof:centroid`],
http://www.opengis.net/def/function/geosparql/concatLines[`geof:concatLines`],
http://www.opengis.net/def/function/geosparql/concaveHull[`geof:concaveHull`],
http://www.opengis.net/def/function/geosparql/convexHull[`geof:convexHull`], 
http://www.opengis.net/def/function/geosparql/union2[`geof:union2`] as a SPARQL extension functions.
...
==== Function: geof:boundingBox
```
geof:boundingBox (geom: ogc:geomLiteral): ogc:geomLiteral
```

This function is an alias for <<Function: geof:envelope, geof:envelope>> and is included due to the common use of the term _Bounding Box_ for an envelope and to keep consistency with _Bounding Circle_.
