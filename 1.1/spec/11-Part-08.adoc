...
`geo:hasDefaultGeometry`, geo:hasLength, geo:hasArea & geo:hasVolume  to be used in SPARQL graph patterns.
...
The property `geo:hasLength` is used to indicate the length of a `geo:Feature`. The property indicates `geo:SpatialMeasure` object which then contains the length value expressed as a scalar number and which potentially also includes units of measure, uncertainty and other properties.
...
An ontological consistency check can be applied to geometries indicating this property and the `geo:dimension` property: the integer range value of a `geo:dimension` property for a geometry with a `geo:hasLength` property must be 1.

===== Property: geo:hasArea

The property `geo:hasArea` is used to indicate the area of a `geo:Feature`. The property indicates `geo:SpatialMeasure` object which then contains the area value expressed as a scalar number and which potentially also includes units of measure, uncertainty and other properties.
...
An ontological consistency check can be applied to geometries indicating this property and the `geo:dimension` property: the integer range value of a `geo:dimension` property for a geometry with a `geo:hasLength` property must be 2.

===== Property: geo:hasVolume

The property `geo:hasVolume` is used to indicate the area of a `geo:Feature`. The property indicates `geo:SpatialMeasure` object which then contains the volume value expressed as a scalar number and which potentially also includes units of measure, uncertainty and other properties.
...
An ontological consistency check can be applied to geometries indicating this property and the `geo:dimension` property: the integer range value of a `geo:dimension` property for a geometry with a `geo:hasLength` property must be 3.
...
The `geo:inCRS` property is used to connect a geometry with its text- based serialization (e.g., its WKT serialization).
