...
This clause establishes the _Geometry Extension_ parameterized requirements class, with IRI `/req/geometry-extension`, which has a single corresponding conformance class _Geometry Extension_, with IRI `/conf/geometry-extension`. This requirements class defines a vocabulary for asserting and querying information about geometry data, and it defines query functions for operating on geometry data.
...
```
geof:DWithin (geom: ogc:geomLiteral, 
                 distance: xsd:double): ogc:geomLiteral
```

This function returns all geometric objects located within a distance measured in meter around `geom`. Calculations are in the spatial reference system of `geom`. The triple store implementation needs to take care of a conversion between the unit given as a parameter and the unit of the spatial rereference system.

==== Function: geof:DWithin

```
geof:DWithin (geom: ogc:geomLiteral, 
                 distance: xsd:double, unit: xsd:anyURI): ogc:geomLiteral
```

This function returns all geometric objects located within a distance measured in a given unit around `geom`. Calculations are in the spatial reference system of `geom`. The triple store implementation needs to take care of a conversion between the unit given as a parameter and the unit of the spatial rereference system.
