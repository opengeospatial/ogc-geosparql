...
|===
| *Req 16* The IRI http://www.opengis.net/def/crs/OGC/1.3/CRS84[`+<http://www.opengis.net/def/crs/OGC/1.3/CRS84>+`] shall be assumed as the spatial reference system for <<RDFS Datatype: geo:wkbLiteral, `geo:wkbLiteral`>> instances that do not specify an explicit spatial reference system IRI.
...
[#req_geometry_extension_wkt-axis-order]
|===
| *Req 17* Coordinate tuples within <<RDFS Datatype: geo:wkbLiteral, `geo:wkbLiteral`>> shall be interpreted using the axis order defined in the spatial reference system used.
...
|===
| *Req 19* Implementations shall allow the RDF property <<Property: geo:asWKB, `geo:asWKB`>> to be used in SPARQL graph patterns.
...
```
geof:asWKB (geom: ogc:geomLiteral): geo:wktLiteral
```
...
|===
| *Req 20* Implementations shall support <<Property: geo:asWKB, `geo:asWKB`>> as a SPARQL extension function.
...
Returns the last point in the given `geom` in the literal format of `geom`.
...
Returns the nth interior ring of `geom` if the geometry represents a Polygon.
...
Returns true if `geom` is a closed geometry, i.e. its start point equals its end point.
...
Returns true if `geom` is a closed geometry (see <<Function: geof:isClosed, geo:isClosed >>) , and simple (see <<Function: geof:isSimple, geo:isSimple >>).
...
Returns the number of interior rings of `geom` if the geometry represents a Polygon.
...
Returns the number of points of `geom`.
...
Returns the first point in the given `geom` in the literal format of  `geom`.
...
This function returns the X coordinate if `geom` is a point geometry. The X coordinate is determined by the X axis definition of its SRS.
...
This function returns the Y coordinate if `geom` is a point geometry. The Y coordinate is determined by the X axis definition of its SRS.
...
This function returns the Z coordinate if `geom` is a point geometry. The Z coordinate is determined by the X axis definition of its SRS.
...
This function returns the M coordinate if `geom` is a point geometry. The M coordinate is determined by the X axis definition of its SRS.
