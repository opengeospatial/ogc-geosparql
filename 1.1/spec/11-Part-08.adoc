...
=== GeoSPARQL and Simple Features

The GeoSPARQL Geometry Extension is largely based on the the specification Simple Features Access - Common Architecture (SFA-CA) <<OGCSFACA>>. Contrary to what the name implies, this specification is about Geometry, not about Features. SFA-CA describes simple geometry, meaning that geometric shapes are based on points and straight lines (lineair interpolations) between points. Within a single Geometry, these lines may not cross.
...
Although the GeoSPARQL Geometry Extension is based on SFA-CA, there are some differences
