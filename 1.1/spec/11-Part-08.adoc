...
This section establishes the Requirements class for representing Discrete Global Grid System (DGGS) Geometry data as RDF literals. The form of representation is specific to individual DGGS implementations: known DGGSes are not compatible or even very similar. 

T,he Requirements class defines one RDFS Datatype
...
An example of a literal for a concrete DGGS, AusPIX, could be
...
NOTE: What `R3234` means, or the meaning of any other element within a concrete DGGS literal is not handled by GeoSPARQL but is expected to be handled by the specification of that DGGS , just as GeoSPARQL does not specify the internals of other Geometry formats such as WKT or GeoJSON.
