...
NOTE: It is unlikely that users of GeoSPARQL will create many instances of <<Class: SpatialObject, Spatial Object>> as its two more concrete subclasses, <<Class: Feature, Feature>> &  <<Class: Geometry, Geometry>> , are more directly relatable to real-world phenomena and use.
...
Here a <<Class: geo:Feature>> is described as per the previous example but its area is expressed in non-metric units. The unit international acre is expressed using the _Quantities, Units, Dimensions and Types (QUDT)_ ontologyfootnote:[http://www.qudt.org]. The use of QUDT and its http://qudt.org/schema/qudt#numericValue[`qudt:numericValue`] & http://qudt.org/schema/qudt#numericValue[`qudt:unit`] is just one of many possible ways to convey the value of a subproperty of <<Property: geo:hasSize>>.
...
In this example it is not possible to convert the length of the feature to meters, because the historical unit does not have a known precise conversion factor. 
