...
Note that the application of http://www.opengis.net/def/function/geosparql/asDGGS[`geof:asDGGS`] requires a `dggsIri` parameter which indicates the IRI of the particular DGGS being converted to. In the case of <<B.1.2.3 Geometry Serializations>>, this value would be `+https://w3id.org/dggs/auspix+`, the IRI of the AusPIX DGGS.
