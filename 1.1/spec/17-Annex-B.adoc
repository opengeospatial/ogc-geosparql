...
This example shows a <<Class: geo:Feature, `geo:Feature`>> instance with each of the properties defined in <<8.3. Standard Properties for geo:Feature>> used, except for the properties
<<Property: geo:hasMetricSize, `geo:hasMetricSize`>> amd <<Property: geo:hasSize, `geo:hasSize`>>, that are intended to be used through their subproperties.
