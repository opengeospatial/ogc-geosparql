...
.
```
This example and the example below (B 1.1.2.3) show the same `Feature`, but with a different specification of its area. This example shows the recommended way to express size: by using a subproperty of `hasMetricSize` (`hasMetricArea` in this case). These subproperties have fixed units based on the SI standard unit of distance: meter.
...
Here a `Feature` and a `Geometry` are described as per the previous example but its area is expressed in non-metric units. Are is expressed using the _Quantities, Units, Dimensions and Types (QUDT)_ ontologyfootnote:[http://www.qudt.org]. The use of QUDT and its http://qudt.org/schema/qudt#numericValue[`qudt:numericValue`] & http://qudt.org/schema/qudt#numericValue[`qudt:unit`] is just one of many possible ways to convey the value of a subproperty of `hasSize`.
...
Machine use of this `Feature` would be able to differentiate the two `Geometry` instances based on this use of http://www.opengis.net/ont/geosparql#hasMetricSpatialResolution[`geo:hasMetricSpatialResolution`].

===== B.1.1.2.7 `Feature` with two different `Geometry` instances using non-metric spatial resolution
