...
|===
| From Element | Mapping relation | To Element | Notes 
| `wdt:P625` | `rowl:equivalentProperty` | `geo:asWKT` | The Wikidata description of this property note that "For Earth, please note that only WGS84 coordinating system is supported at the moment" but that is a system limit, not an ontological one
| `wdt:P3896`   | `owl:propertyChainAxiom` | `(geo:hasGeometry geo:asGeoJSON)` | This Wikidata property indicated GeoJSON geomettry literal content for a Feature, but it allows information other than just Geometry in the GeoJSON whereas GeoSPARQL does not.
| `wdt:Q82794` | `geo:Feature` | `rdfs:subClassOf` | The Wikidata property is labeled "geographic region" and thus is a subclass of the more general `geo:Feature`. There are likely many other classes in Wikidata that could be interpreted as subclasses of `geo:Feature`
...
| `geo:sfContains` | `rdfs:subPropertyOf` | `wdt:Q65964571` | Inverse of `wdt:P361` (see above)
