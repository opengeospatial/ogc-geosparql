...
| `prov:atLocation` | - | - | The PROV property indicates `prov:Location` instances, which may be `geo:Feature` instnaces, but GeoSPARQL has no property to indicate a `geo:Feature`, so no mapping is possible. Indicating features is commonly done in ontologies used GeoSPARQL but not within GeoSPARQL.
|===
