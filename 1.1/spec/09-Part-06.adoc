...
Three main classes are defined: `geo:SpatialObject`, `geo:SpatialMeasure` and `geo:Feature`. The class `geo:Feature` is equivalent to the UML class `Feature` defined in <<19109>>.
...
The class `geo:Feature` is equivalent to the class `GFI_Feature` <<19156>> and is defined by the following:
