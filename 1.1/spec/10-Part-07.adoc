...
This clause establishes the _Topology Vocabulary Extension_ parameterized Requirements class with the IRI base `/req/topology-vocab-extension`, which has a single corresponding Conformance Class _Topology Vocabulary Extension_, with IRI `/conf/topology-vocab-extension`. This Requirements class defines a vocabulary for asserting and querying topological relations between spatial objects. The class is parameterized so that different families of topological relations may be used, e.g. RCC8 and Egenhofer. These relations are generalized so that they may connect features as well as geometries.
...
This clause defines Requirements for the 9-intersection model for the binary topological relations (_Egenhofer_) relation family. Consult references <<FORMAL>> and <<CATEG>> for a more detailed discussion of _Egenhofer_ relations.
