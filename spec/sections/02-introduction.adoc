...
The W3C Semantic Web Activity defines a collection of technologies that enables a “web of data” where information is easily shared and reused across applications. Some key pieces of this technology stack are the Resource Description Framework (RDF) data model <<RDF>>, <<RDFS>>, the OWL Web Ontology Language cite:[OWL2] and the SPARQL Protocol and RDF Query Language <<SPARQL>>.
...
Any of the three values in a triple can be represented with an Internationalized Resource Identifier (IRI) cite:[IETF3987], which globally and uniquely identifies the resource referenced. IRIs are an extension to Uniform Resource Identifiers (URIs) that allow for non-ASCII characters. In addition to functioning as identifiers, IRIs are usually, but not necessarily, resolvable. This means a person or machine can "dereference" them (_click on them_ or otherwise execute them) and be taken to more information about the resource, perhaps in a web browser.

Subjects and objects within an RDF triple are called nodes and can also be represented with a blank node (a local identifier without meaning outside the graph it is defined within). Objects can further be represented with a literal value. RDF uses the basic literal values from XML cite:[XSD2] however the basic types can be extended for specialized purposes. Indeed, this document extends the basic types to include geometry data. The figure below shows a basic triple.
...
Almost all data can be presented or represented in RDF. In particular, there are similarities to the (feature-instance-by-id, attribute, value) tuples of the General Feature Model cite:[ISO19109], and to the relational model as well (table primary key, column, value).
...
GeoSPARQL does not directly provide support for temporality. Predicates for temporal relations may be used from the OWL Time Ontology cite:[TIME], but query extension functions for spatiotemporal operations are not present in the GeoSPARQL standard.

[[GeoSPARQL_Standard_structure]]
...
** Formally defined as an ontology, defined according to the _Profiles Vocabulary_ cite:[PROF];
...
** **Includes normative RDF/OWL <<RDF>>,cite:[OWL2] ontology element definitions, conformance requirements and function signatures based on the General Feature Model cite:[ISO19109], Simple Features cite:[OGCSFACA] cite:[ISO19125-1] and SQL MM cite:[ISO13249];**
** **Also includes non-normative examples and mappings to other modelling and function systems.**
3. Domain model RDF/OWL <<RDF>>,cite:[OWL2] ontology
** http://www.opengis.net/ont/geosparql;
** For geographic information representation;
** Based on the General Feature Model cite:[ISO19109], Simple Features Access cite:[OGCSFACA] cite:[ISO19125-1], Geography Markup Language <<GML>> and SQL MM cite:[ISO13249]
** Defined within the specification document and also delivered in RDF.
4. Functions & Rules vocabularies
** http://www.opengis.net/def/function/geosparql;
** http://www.opengis.net/def/rule/geosparql;
** Derived from the ontology;
** Presented as a cite:[SKOS] taxonomy.
5. Simple Features vocabulary
** http://www.opengis.net/ont/sf;
** Derived from the class model defined in Simple Features Access cite:[OGCSFACA] cite:[ISO19125-1];
** Presented as an cite:[OWL2] ontology.
...
** Defined using cite:[SPARQLSERVDESC].
9. Extended Examples
