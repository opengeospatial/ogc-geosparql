...
This annex presents a mapping between the Common Query Language(CQL) cite:[CQLDEF] and GeoSPARQL as well as generic SPARQL <<SPARQL>>. This is likely of relevance to the delivery of GeoSPARQL data via systems such as the OGC's Web Feature Service cite:[WFS] and OGC API Features cite:[OGCAPIF] which implement CQL.
