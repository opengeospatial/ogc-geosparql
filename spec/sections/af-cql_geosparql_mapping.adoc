...
Spatial _Features_ accessed via SPARQL endpoints <<SPARQLPROT>> are, as defined in the GeoSPARQL standard, instances of the OWL class <<class-feature, `Feature`>> or of subclasses of it.
They may have one or more <<prop-hasgeometry, `hasGeometry`>> properties indicating <<class-geometry, `Geometry`>> instances and other properties related to the Feature. They may also be grouped into <<class-featurecollection, `FeatureCollection`>> instances where <<class-featurecollection, `FeatureCollection`>> is a new class in GeoSPARQL 1.1, specifically for the description of collections of <<class-feature, `Feature`>> instances.
...
GeoSPARQL's <<class-featurecollection, `FeatureCollection`>> definition requires that <<class-feature, `Feature`>> instances are to be linked to the Collection by use of the http://www.w3.org/1999/02/22-rdf-syntax-ns#member[``rdf:member``] property. No inverse property is defined.
...
* APIs may need more information about the <<class-featurecollection, `FeatureCollection`>> instance for correct handling, in particular, an identifier and perhaps a label. If the back-end data store also contains information for the <<class-featurecollection, `FeatureCollection`>> instance then this may be queried for. If not, the API might need to create such data
...
| Spatial Geometry (WKT)  | POINT(1 1) | <<datatype-wktliteral, `wktLiteral`>> | "POINT(1 1)"^^geo:wktLiteral
| Spatial Geometry (JSON)  | {"type": "Point", "coordinates":[1,1]} | <<datatype-geojsonliteral, `geoJSONLiteral`>> | "{"type": "Point", "coordinates":[1,1]}"^^geo:geoJSONLiteral
...
As noted above in Section <<query-parameters>>, temporal filtering of RDF data via SPARQL queries is possible with standard SPARQL functions to compare date values (http://www.w3.org/2001/XMLSchema#date[`xsd:date`], http://www.w3.org/2001/XMLSchema#dateTime[`xsd:dateTime`] and http://www.w3.org/2001/XMLSchema#dateTimeStamp[`xsd:dateTimeStamp`] literals) and OWL TIME <<TIME>> may be used to assert temporal relations between objects.
