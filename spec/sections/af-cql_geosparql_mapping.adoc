...
This annex presents a mapping between the Common Query Language (CQL) <<CQL>> and GeoSPARQL as well as generic SPARQL <<SPQL, W3C REC-sparql11-query-20130321>>. This is likely of relevance to the delivery of GeoSPARQL data via systems such as the Web Feature Service <<WFS>> and OGC API Features <<OAF>> which implement CQL.

=== Accessing spatial Features in a SPARQL endpoint

Spatial _Features_ accessed via SPARQL endpoints <<SPQLP>> are, as defined in the GeoSPARQL standard, instances of the OWL class <<Feature, Feature>> or of subclasses of it.
They may have one or more <<hasGeometry>> properties indicating <<Geometry, Geometry>> instances and other properties related to the Feature. They may also be grouped into <<FeatureCollection, FeatureCollection>> instances where `FeatureCollection` is a new class in GeoSPARQL 1.1, specifically for the description of collections of <<Feature, Feature>> instances.
...
GeoSPARQL's <<FeatureCollection, FeatureCollection>> definition requires that <<Feature, Feature>> instances are to be linked to the Collection by use of the http://www.w3.org/1999/02/22-rdf-syntax-ns#member[``rdf:member``] property. No inverse property is defined.
...
* APIs may need more information about the <<FeatureCollection, FeatureCollection>> instance for correct handling, in particular, an identifier and perhaps a label. If the back-end data store also contains information for the <<FeatureCollection, FeatureCollection>> instance then this may be queried for. If not, the API might need to create such data
...
This section presents lists of equivalences between Common Query Language (CQL2) <<CQL>> statements and GeoSPARQL statements.

[[cql_query_parameters]]
==== Query Parameters
...
As noted above in Section <<cql_query_parameters, F.2.1 Query Parameters>>, temporal filtering of RDF data via SPARQL queries is possible with standard SPARQL functions to compare date values (http://www.w3.org/2001/XMLSchema#date[`xsd:date`], http://www.w3.org/2001/XMLSchema#dateTime[`xsd:dateTime`] and http://www.w3.org/2001/XMLSchema#dateTimeStamp[`xsd:dateTimeStamp`] literals) and OWL TIME <<TIME>> may be used to assert temporal relations between objects.

=== Mappings from Simple Features for SQL

The following table maps the functions and properties from Simple Features for SQL <<SFA>> <<WKT>> to GeoSPARQL.
