...
This annex presents a mapping between the Common Query Language (CQL) <<OGC_19-079r2>> and GeoSPARQL as well as generic SPARQL <<SPARQL>>. This is likely of relevance to the delivery of GeoSPARQL data via systems such as the Web Feature Service <<OGC_09-025r2>> and OGC API Features <<OGC_17-069r4>> which implement CQL.

=== Accessing spatial Features in a SPARQL endpoint

Spatial _Features_ accessed via SPARQL endpoints <<W3C_REC-sparql11-protocol-20130321>> are, as defined in the GeoSPARQL standard, instances of the OWL class <<Class: geo:Feature, `geo:Feature`>> or of subclasses of it.
...
This section presents lists of equivalences between Common Query Language (CQL2) <<OGC_19-079r2>> statements and GeoSPARQL statements.
...
| datetime | ``datetime= 2018-02-12 T23%3A20%3A52Z`` | - | - | GeoSPARQL doesn't detail temporal aspects of data. Filtering data using RDF temporal properties may be achieved using basic SPARQL queries and also OWL TIME <<W3C_CRD-owl-time-20221115>>
|===
...
As noted above in Section <<cql_query_parameters, F.2.1 Query Parameters>>, temporal filtering of RDF data via SPARQL queries is possible with standard SPARQL functions to compare date values (http://www.w3.org/2001/XMLSchema#date[`xsd:date`], http://www.w3.org/2001/XMLSchema#dateTime[`xsd:dateTime`] and http://www.w3.org/2001/XMLSchema#dateTimeStamp[`xsd:dateTimeStamp`] literals) and OWL TIME <<W3C_CRD-owl-time-20221115>> may be used to assert temporal relations between objects.

=== Mappings from Simple Features for SQL

The following table maps the functions and properties from Simple Features for SQL <<OGC_06-103r4>> <<ISO_19125-1:2004>> to GeoSPARQL.
