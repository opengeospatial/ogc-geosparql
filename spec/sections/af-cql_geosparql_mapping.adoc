...
This annex presents a mapping between the Common Query Language (CQL) cite:[CQLDEF] and GeoSPARQL as well as generic SPARQL cite:[SPARQL]. This is likely of relevance to the delivery of GeoSPARQL data via systems such as the OGC's Web Feature Service cite:[WFS] and OGC API Features cite:[OGCAPIF] which implement CQL.

=== Accessing spatial Features in a SPARQL endpoint

Spatial _Features_ accessed via SPARQL endpoints cite:[SPARQLPROT] are, as defined in the GeoSPARQL standard, instances of the OWL class <<Class: geo:Feature, `geo:Feature`>> or of subclasses of it.
...
This section presents lists of equivalences between Common Query Language (CQL2) cite:[CQLDEF] statements and GeoSPARQL statements.
...
| datetime | ``datetime= 2018-02-12 T23%3A20%3A52Z`` | - | - | GeoSPARQL doesn't detail temporal aspects of data. Filtering data using RDF temporal properties may be achieved using basic SPARQL queries and also OWL TIME cite:[TIME]
|===
...
As noted above in Section <<cql_query_parameters, F.2.1 Query Parameters>>, temporal filtering of RDF data via SPARQL queries is possible with standard SPARQL functions to compare date values (http://www.w3.org/2001/XMLSchema#date[`xsd:date`], http://www.w3.org/2001/XMLSchema#dateTime[`xsd:dateTime`] and http://www.w3.org/2001/XMLSchema#dateTimeStamp[`xsd:dateTimeStamp`] literals) and OWL TIME cite:[TIME] may be used to assert temporal relations between objects.

=== Mappings from Simple Features for SQL

The following table maps the functions and properties from Simple Features for SQL cite:[OGCSFACA] cite:[ISO19125-1] to GeoSPARQL.
