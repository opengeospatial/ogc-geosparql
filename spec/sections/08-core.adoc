...
.SPARQL Protocol
====
Implementations shall support the SPARQL Query Language for RDF <<SPARQL>>, the SPARQL Protocol cite:[SPARQLPROT] and the SPARQL Query Results XML cite:[SPARQLRESX] and JSON cite:[SPARQLRESJ] Formats.
====
...
The class http://www.opengis.net/ont/geosparql#Feature[`geo:Feature`] is equivalent to the class `GFI_Feature` cite:[ISO19156] and is defined by the following:
