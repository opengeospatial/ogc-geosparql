...
.SPARQL Protocol
====
Implementations shall support the SPARQL Query Language for RDF <<SPARQL>>, the SPARQL Protocol <<W3C_REC-sparql11-protocol-20130321>> and the SPARQL Query Results XML <<W3C_REC-rdf-sparql-XMLres-20130321>> and JSON <<W3C_REC-sparql11-results-json-20130321>> Formats.
====
...
The class http://www.opengis.net/ont/geosparql#Feature[`geo:Feature`] is equivalent to the class `GFI_Feature` <<ISO_19156:2011>> and is defined by the following:
...
Membership of the generic  http://www.w3.org/2000/01/rdf-schema#Container[`rdfs:Container`] that defines this class is restricted to instances of <<Class: SpatialObject, Spatial Object>>. <<Class: SpatialObjectCollection, Spatial Object Collection>> members are to be indicated with the  http://www.w3.org/2000/01/rdf-schema#member[`rdfs:member`] property.
...
Membership of the more general <<Class: SpatialObjectCollection, Spatial Object Collection>> that defines this class is restricted to instances of <<Class: Feature, Feature>>. <<Class: geo:FeatureCollection, `geo:FeatureCollection`>> members are to be indicated with the `rdfs:member` property.
