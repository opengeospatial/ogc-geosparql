...
Here a <<Class: geo:Feature, `geo:Feature`>> is declared and linked to a <<Class: geo:GeometryCollection, `geo:GeometryCollection>> in order to bundle multiple <<Class: geo:Geometry, `geo:Geometry`>> instances together. Using this mechanism, you could add additional properties to the <<Class: geo:GeometryCollection, `geo:GeometryCollection>> to detail useful things about it. This is an alternative to the pattern given in <<C.1.1.2.5, `Feature` with two different `Geometry` instances indicated>> example below.
