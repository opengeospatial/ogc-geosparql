[appendix,id=annexc,obligation=informative]
== Examples
...
[example]
===== `SpatialObject`
The `SpatialObject` class is defined in <<class-spatialobject, `SpatialObject`>>.
...
NOTE: It is unlikely that users of GeoSPARQL will create many instances of <<class-spatialobject, `SpatialObject`>> as it has two, more concrete, subclasses, <<class-feature, `Feature`>> and <<class-geometry, `Geometry`>>, which are more directly relatable to real-world phenomena and use.

[[C.1.1.1.2]]
[example]
====== Size Properties

The "size" properties - <<prop-hassize, `hasSize`>>,
...
<<prop-hasmetricvolume, `hasMetricVolume`>> - are all applicable to instances of <<class-spatialobject, `SpatialObject`>> although, as per the note in the section above, they are likely to be used with <<class-feature, `Feature`>> and <<class-geometry, `Geometry`>> instances.
...
Here a spatial object, Moreton Island, has the distance of its coastline given with two properties: <<prop-hasperimeterlength, `hasPerimeterLength`>> and <<prop-hasmetricperimeterlength, `hasMetricPerimeterLength`>>. The object for the first is a Blank Node with a QUDT value property of 92.367 and a QUDT unit property of `unit:KiloM` (kilometre). The object for the second is the literal `92367` (a double) which is, by the property's definition, a number of metres.

The use of the _Quantities, Units, Dimensions and Types (QUDT)_ ontologyfootnote:[http://www.qudt.org] and its http://qudt.org/schema/qudt#numericValue[`qudt:numericValue`] and http://qudt.org/schema/qudt#numericValue[`qudt:unit`] is just one of many possible ways to convey the value of <<prop-hasperimeterlength, `hasPerimeterLength`>> and any subproperty of <<prop-hassize, `hasSize`>>.

[[C.1.1.2]]
[example]
===== `Feature`
The `Feature` class is defined in <<class-feature, `Feature`>>.
...
Here a <<class-feature, `Feature`>> is declared, given a preferred label and a Geometry for that <<class-feature, `Feature`>> is indicated with the use of <<prop-hasgeometry, `hasGeometry`>>. The `Geometry` indicated is described using a _Well-Known Text_ literal value, indicated by the property <<prop-aswkt, `asWKT`>> and the literal type <<datatype-wktliteral, `wktLiteral`>>.
...
Here a <<class-feature, `Feature`>> is described as per the previous example but its area is expressed in non-metric units: the acre.
...
Machine use of this `Feature` would be able to differentiate the two `Geometry` instances based on this use of <<prop-hasmetricspatialresolution, `hasMetricSpatialResolution`>>.
...
[example]
===== `Geometry`
The `Geometry` class is defined in <<class-geometry, `Geometry`>>.
...
.
----
This example shows a Feature, `eg:x`, with a Geometry, `eg:x-geo`, which has both a serialization (WKT) indicated with the predicate <<prop-aswkt, `asWKT`>> and a scalar area indicated with the predicate <<prop-hasmetricarea, `hasMetricArea`>>. While it is entirely possible that scalar areas can be calculated from polygons, it may be efficient to store a pre-calculated scalar area in addition to the polygon. Perhaps the polygon is large and detailed and a one-time calculation with results stored is efficient for repeated use.

This use of a scalar spatial measurement property with a Geometry, here <<prop-hasmetricarea, `hasMetricArea`>>, is possible since the domain of such properties is <<class-spatialobject, `SpatialObject`>>, the superclass of both <<class-feature, `Feature`>> and <<class-geometry, `Geometry`>>.

[example]
===== `SpatialObjectCollection`

<<class-spatialobjectcollection, `spatialobjectcollection`>> isn't really intended to be implemented - it's essentially an abstract class - therefore no examples of its use are given. See the following two sections for examples of the concrete <<class-featurecollection, `featurecollection`>> and <<class-geometrycollection, `geometrycollection`>> classes.
...
There are several GeoSPARQL properties that suggest they could be used with particular _Simple Features Vocabulary_ geometry types, for instance, <<prop-hascentroid, `hasCentroid`>> indicates is could be used with a http://www.opengis.net/ont/sf#Point[`sf:Point`] and <<prop-hasboundingbox, `hasBoundingBox`>> indicates use with an `sf:Envelope`.

==== Properties

===== Spatial Object Properties

See the section <<C.1.1.1.2>> above.

[example]
===== Feature Properties

This example shows a <<class-feature, `Feature`>> instance with each of the properties defined in <<feature_properties>> used, except for the properties
<<prop-hasmetricsize, `hasMetricSize`>> and <<prop-hassize, `hasSize`>>, that are intended to be used through their subproperties and <<prop-hasmetricperimeterlength, `hasMetricPerimeterLength`>> and <<prop-hasperimeterlength, `hasPerimeterLength`>> which are examplified in <<C.1.1.1.2>>.
...
[example]
=== Example SPARQL Queries and Rules
...
//image::figures/03.png[600,400,align="center"]
...
For the geometry literal values in <<Geometry Serializations>>:

Application of the function http://www.opengis.net/def/function/geosparql/asWKT[`geof:asWKT`] to the GML, KML, GeoJSON and DGGS literals should return WKT literal and similarly for each of the other conversion methods, http://www.opengis.net/def/function/geosparql/asGML[`geof:asGML`], http://www.opengis.net/def/function/geosparql/asKML[`geof:asKML`], http://www.opengis.net/def/function/geosparql/asGeoJSON[`geof:asGeoJSON`] and http://www.opengis.net/def/function/geosparql/asDGGS[`geof:asDGGS`].
