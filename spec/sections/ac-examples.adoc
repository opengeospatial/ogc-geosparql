...
[[C.1.1.1]]
===== Spatial Object
The `SpatialObject` class is defined in <<SpatialObject, SpatialObject>>.
...
NOTE: It is unlikely that users of GeoSPARQL will create many instances of <<SpatialObject, SpatialObject>> as its two more concrete subclasses, <<Feature, Feature>> & <<Geometry, Geometry>>, are more directly relatable to real-world phenomena and use.

[[C.1.1.1.2]]
====== Size Properties

The "size" properties - <<hasSize>>,
<<hasMetricSize>>,
<<hasLength>>,
<<hasMetricLength>>,
<<hasPerimeterLength>>,
...
<<hasMetricArea>>,
<<hasVolume>> and
<<hasMetricVolume>> - are all applicable to instances of <<SpatialObject, SpatialObject>> although, as per the note in the section above, they are likely to be used with <<Feature, Feature>> & <<Geometry, Geometry>> instances.
...
Here a spatial object, Moreton Island, has the distance of its coastline given with two properties: <<hasPerimeterLength>> & <<hasMetricPerimeterLength>>. The object for the first is a Blank Node with a QUDT value property of 92.367 and a QUDT unit property of `unit:KiloM` (kilometre). The object for the second is the literal `92367` (a double) which is, by the property's definition, a number of metres.

The use of the _Quantities, Units, Dimensions and Types (QUDT)_ ontologyfootnote:[http://www.qudt.org] and its http://qudt.org/schema/qudt#numericValue[`qudt:numericValue`] & http://qudt.org/schema/qudt#numericValue[`qudt:unit`] is just one of many possible ways to convey the value of <<hasPerimeterLength>> and any subproperty of <<hasSize>>.

[[C.1.1.2]]
===== `Feature`
The `Feature` class is defined in <<Feature, Feature>>.
...
Here a <<Feature, Feature>> is declared, given a preferred label and a Geometry for that <<Feature, Feature>> is indicated with the use of <<hasGeometry>>. The `Geometry` indicated is described using a _Well-Known Text_ literal value, indicated by the property <<asWKT>> and the literal type `geo:wktLiteral`.
...
.
----
This example and the example below (B 1.1.2.4) show the same <<Feature, Feature>>, but with a different specification of its area. This example shows the recommended way to express size: by using a subproperty of <<hasMetricSize>> (in this case, <<Property: hasMetricArea>>). These subproperties have fixed units based on meter (the unit of distance in the International System of Units).
...
Here a <<Feature, Feature>> is described as per the previous example but its area is expressed in non-metric units: the acre.
...
Machine use of this `Feature` would be able to differentiate the two `Geometry` instances based on this use of <<hasMetricSpatialResolution>>.
...
[[C.1.1.3]]
===== `Geometry`
The `Geometry` class is defined in <<Geometry, Geometry>>.
...
Here a single `Geometry`, linked to a `Feature` instance, is expressed using two different serializations: Well-known Text and the DGGS with the AusPIX DGGS <<AUSPIX>> indicated by its IRI.
...
This use of a scalar spatial measurement property with a Geometry, here <<hasMetricArea>>, is possible since the domain of such properties is <<SpatialObject, SpatialObject>>, the superclass of both <<Feature, Feature>> and <<Geometry, Geometry>>.

[[C.1.1.4]]
===== `SpatialObjectCollection`

<<SpatialObjectCollection, SpatialObjectCollection>> isn't really intended to be implemented - it's essentially an abstract class - therefore no examples of its use are given. See the following two sections for examples of the concrete <<FeatureCollection, FeatureCollection>> & <<GeometryCollection, GeometryCollection>> classes.
...
There are several GeoSPARQL properties that suggest they could be used with particular _Simple Features Vocabulary_ geometry types, for instance, <<hasCentroid>> indicates is could be used with a http://www.opengis.net/ont/sf#Point[`sf:Point`] and <<hasBoundingBox>> indicates use with an `sf:Envelope`.
...
This example shows a <<Feature, Feature>> instance with each of the properties defined in <<Standard Properties for geo:Feature>> used, except for the properties
<<hasMetricSize>> and <<hasSize>>, that are intended to be used through their subproperties and <<hasMetricPerimeterLength>> and <<hasPerimeterLength>> which are exemplified in <<C.1.1.1.2>>.
...
In this example, each of the properties defined for a `Geometry` instance has realistic values. For example, the <<Property: isEmpty, is empty>> property is set to `false` since the `Geometry` contains information.
...
*Result*:
[%unnumbered]
.Result set for containing features
|===
|*?f*
...
*Result*:
[%unnumbered]
.Result set for features within the bounding box
|===
|*?f*
...
*Result*:
[%unnumbered]
.Result set for touching features
|===
|*?f*
...
*Result*:
[%unnumbered]
.Result set for closest features
|===
|*?f*
...
*Result*:
[%unnumbered]
.Result set for bounding coordinates
|===
...
*Result*:
[%unnumbered]
.Result set for intersecting features
|===
|*?f*
...
For the geometry literal values in <<C.1.2.4>>:
