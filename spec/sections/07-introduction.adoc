...
The W3C Semantic Web Activity defines a collection of technologies that enables a “web of data” where information is easily shared and reused across applications. Some key pieces of this technology stack are the Resource Description Framework (RDF) data model <<W3C_REC-rdf11-concepts-20140225>>, <<W3C_REC-rdf-schema-20140225>>, the OWL Web Ontology Language <<W3C_REC-owl2-overview-20121211>> and the SPARQL Protocol and RDF Query Language <<SPARQL>>.
...
Any of the three values in a triple can be represented with an Internationalized Resource Identifier (IRI) <<IETF_RFC_3987>>, which globally and uniquely identifies the resource referenced. IRIs are an extension to Uniform Resource Identifiers (URIs) that allow for non-ASCII characters. In addition to functioning as identifiers, IRIs are usually, but not necessarily, resolvable. This means a person or machine can "dereference" them (_click on them_ or otherwise execute them) and be taken to more information about the resource, perhaps in a web browser.

Subjects and objects within an RDF triple are called nodes and can also be represented with a blank node (a local identifier without meaning outside the graph it is defined within). Objects can further be represented with a literal value. RDF uses the basic literal values from XML <<W3C_REC-xmlschema-2-20041028>> however the basic types can be extended for specialized purposes. Indeed, this document extends the basic types to include geometry data. The figure below shows a basic triple.
...
Almost all data can be presented or represented in RDF. In particular, there are similarities to the (feature-instance-by-id, attribute, value) tuples of the General Feature Model <<ISO_19109>>, and to the relational model as well (table primary key, column, value).
...
GeoSPARQL does not directly provide support for temporality. Predicates for temporal relations may be used from the OWL Time Ontology <<W3C_CRD-owl-time-20221115>>, but query extension functions for spatiotemporal operations are not present in the GeoSPARQL standard.
...
** Formally defined as an ontology, defined according to the _Profiles Vocabulary_ <<W3C_NOTE-dx-prof-20191218>>;
...
** **Includes normative RDF <<W3C_REC-rdf11-concepts-20140225>> & OWL ontology element definitions, conformance requirements and function signatures based on the General Feature Model <<ISO_19109>>, Simple Features <<OGC_06-103r4>> <<ISO_19125-1:2004>> and SQL <<ISOIEC_13249-3>>;**
** **Also includes non-normative examples and mappings to other modelling and function systems.**
3. Domain model RDF/OWL schema
...
** Presented as SKOS <<W3C_REC-skos-reference-20090818>> taxonomies.
5. Simple Features vocabulary
** http://www.opengis.net/ont/sf;
** Derived from the class model defined in Simple Features Access <<OGC_06-103r4>> <<ISO_19125-1:2004>>;
** Presented as an OWL <<W3C_REC-owl2-overview-20121211>> ontology.
...
** Defined using SHACL <<W3C_REC-shacl-20170720>>;
...
** Defined using <<W3C_REC-sparql11-service-description-20130321>>.
9. Extended Examples
