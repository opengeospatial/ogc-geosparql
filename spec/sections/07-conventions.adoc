...
Three RDF serializations are used in this document. Terse RDF Triple Language (turtle) <<TURTLE>> is used for RDF snippets placed within the main body of the document. Turtle, JSON-LD <<JSON-LD>> and RDF/XML <<RDFXML>> are used for the examples in <<annexc>>.
