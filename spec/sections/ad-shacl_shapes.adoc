...
The SHACL Shapes provided with GeoSPARQL, see <<GeoSPARQL Standard structure>>, are used to verify the graph structure of GeoSPARQL graphs.
...
| Violation
| Each node with an incoming <<prop-hasgeometry, `hasGeometry`>>, or a specialization of it, should have at minimum one outgoing relation that is either <<prop-hasserialization, `hasSerialization`>>, or a specialization of it.
| <<geometry_properties>>, <<req_geometry-extension_feature-properties>>

|http://www.opengis.net/def/geosparql/validator/S1-b-hasGeometry-hasSerialization-sub[Shape 1b]
| Violation
| Each node with an incoming <<prop-hasgeometry, `hasGeometry`>>, or a specialization of it, can have a maximum of one outgoing <<prop-aswkt, `asWKT`>> relation.
| <<geometry_properties>> <<req_geometry_extension_geometry-as-wkt-literal>>

|http://www.opengis.net/def/geosparql/validator/S1-c-hasGeometry-hasSerialization-sub[Shape 1c]
| Violation
| Each node with an incoming <<prop-hasgeometry, `hasGeometry`>>, or a specialization of it, can have a maximum of one outgoing <<prop-asgml, `asGML`>> relation.
| <<geometry_properties>> <<req_geometry_extension_geometry-as-gml-literal>>

|http://www.opengis.net/def/geosparql/validator/S1-d-hasGeometry-hasSerialization-sub[Shape 1d]
| Violation 
| Each node with an incoming <<prop-hasgeometry, `hasGeometry`>>, or a specialization of it, can have a maximum of one outgoing <<prop-asgeojson, `asGeoJSON`>>  relation.
| <<geometry_properties>> <<req_geometry_extension-as-geojson-literal>>

|http://www.opengis.net/def/geosparql/validator/S1-e-hasGeometry-hasSerialization-sub[Shape 1e]
| Violation
| Each node with an incoming <<prop-hasgeometry, `hasGeometry`>>, or a specialization of it, can have a maximum of one outgoing <<prop-askml, `asKML`>> relation.
| <<geometry_properties>> <<req_geometry_extension_geometry-as-kml-literal>>

|http://www.opengis.net/def/geosparql/validator/S2-hasSerialization-hasGeometry[Shape 2]
| Violation
| Each node with one or more outgoing relations that are either <<prop-hasserialization, `hasSerialization`>>, or a specialization of it, should have at least one incoming <<prop-hasgeometry, `hasGeometry`>> relation or a specialization of it.
| <<geometry_properties>>

|http://www.opengis.net/def/geosparql/validator/S3-hasGeometry-hasGeometry[Shape 3a-c]
| Violation
| A node that has an incoming <<prop-hasgeometry, `hasGeometry`>> property, or specialization of it, cannot have an outgoing <<prop-hasgeometry, `hasGeometry`>> property, or a specialization of, it at the same time (a <<class-feature, `Feature`>> cannot be a <<class-geometry, `Geometry`>> at the same time)
...
| Violation
| The target of a <<prop-hasserialization, `hasSerialization`>> property, or a specialization of, it should be an RDF literal
| <<geometry_properties>>

|http://www.opengis.net/def/geosparql/validator/S5-asWKT-wktLiteral[Shape 5]
| Violation
| The target of a <<prop-aswkt, `asWKT`>> property should be an RDF literal with datatype <<datatype-wktliteral, `wktLiteral`>>
| <<req_geometry_extension_wkt-literal>>

|http://www.opengis.net/def/geosparql/validator/S6-asGML-gmlLiteral[Shape 6]
| Violation
| The target of a <<prop-asgml, `asGML`>> property should be an RDF literal with datatype <<datatype-gmlliteral, `gmlLiteral`>>
| <<req_geometry_extension_gml-literal>>

|http://www.opengis.net/def/geosparql/validator/S7-asGeoJSON-geoJSONLiteral[Shape 7]
| Violation
| The target of a <<prop-asgeojson, `asGeoJSON`>> property should be an RDF literal with datatype <<datatype-geojsonliteral, `geoJSONLiteral`>>
| <<req_geometry_extension_geojson-literal>>

|http://www.opengis.net/def/geosparql/validator/S8-asKML-kmlLiteral[Shape 8]
| Violation
| The target of a <<prop-askml, `asKML`>> property should be an RDF literal with datatype <<datatype-kmlliteral, `kmlLiteral`>>
| <<req_geometry_extension_kml-literal>>

|http://www.opengis.net/def/geosparql/validator/S9-many-coordinateDimension-one[Shape 9]
| Violation
| A <<class-geometry, `Geometry`>> node should have a maximum of one outgoing <<prop-coordinatedimension, `coordinateDimension`>> property
| <<geometry_properties>>

|http://www.opengis.net/def/geosparql/validator/S10-many-dimension-one[Shape 10]
| Violation
| A <<class-geometry, `Geometry`>> node should have a maximum of one outgoing <<prop-dimension, `dimension`>> property
| <<geometry_properties>>

|http://www.opengis.net/def/geosparql/validator/S11-many-isEmpty-one[Shape 11]
| Violation
| A <<class-geometry, `Geometry`>> node should have a maximum of one outgoing <<prop-isempty, `isEmpty`>> property
| <<geometry_properties>>

|http://www.opengis.net/def/geosparql/validator/S12-many-isSimple-one[Shape 12]
| Violation
| A <<class-geometry, `Geometry`>> node should have a maximum one outgoing <<prop-issimple, `isSimple`>> property
| <<geometry_properties>>

|http://www.opengis.net/def/geosparql/validator/S13-many-hasSpatialDimension-one[Shape 13]
| Violation
| A <<class-geometry, `Geometry`>> node should have maximum of one outgoing <<prop-hasspatialdimension, `hasSpatialDimension`>> property
| <<geometry_properties>>

|http://www.opengis.net/def/geosparql/validator/S14a-many-hasSpatialResolution-one[Shape 14a]
| Violation
| A <<class-geometry, `Geometry`>> node should have maximum of one outgoing <<prop-hasspatialresolution, `hasSpatialResolution`>> property
| <<geometry_properties>>

|http://www.opengis.net/def/geosparql/validator/S14b-many-hasSpatialAccuracy-one[Shape 14b]
| Violation
| A <<class-geometry, `Geometry`>> node should have maximum of one outgoing <<prop-hasspatialaccuracy, `hasSpatialAccuracy`>> property
| <<geometry_properties>>

|http://www.opengis.net/def/geosparql/validator/S14c-many-hasMetricAccuracy-one[Shape 14c]
| Violation
| A <<class-geometry, `Geometry`>> node should have maximum of one outgoing <<prop-hasmetricspatialaccuracy, `hasMetricSpatialAccuracy`>> property
| <<geometry_properties>>

|http://www.opengis.net/def/geosparql/validator/S14d-many-hasMetricResolution-one[Shape 14d]
| Violation
| A <<class-geometry, `Geometry`>> node should have maximum of one outgoing <<prop-hasmetricspatialresolution, `hasMetricSpatialResolution`>> property
| <<geometry_properties>>

|http://www.opengis.net/def/geosparql/validator/S15-wkt-content[Shape 15]
| Violation
| The content of an RDF literal with an incoming <<prop-aswkt, `asWKT`>> relation must conform to a well-formed WKT string, as defined by its official specification (Simple Features Access)
| <<req_geometry_extension_wkt-literal>>
...
| Violation
| The content of an RDF literal with an incoming <<prop-asgeojson, `asGeoJSON`>> relation must conform to a well-formed GeoJSON geometry string, as defined by its official specification
| <<req_geometry_extension_geojson-literal>>

|http://www.opengis.net/def/geosparql/validator/S18-kml-content[Shape 18]
| Violation
| The content of an RDF literal with an incoming <<prop-askml, `asKML`>> relation must conform to a well-formed KML geometry XML string, as defined by its official specification
| <<req_geometry_extension_kml-literal>>

|http://www.opengis.net/def/geosparql/validator/S20-dimension-coordinateDimension[Shape 20]
| Violation
| If both geo:dimension and <<prop-coordinatedimension, `coordinateDimension`>> properties are asserted, the value of <<prop-dimension, `dimension`>> should be less than or equal to the value of geo:coordinateDimension
| <<geometry_properties>>

|http://www.opengis.net/def/geosparql/validator/S21-FeatureCollectionClass-minOneMember-feature-sub[Shape 21a]
| Violation
| An instance of <<class-featurecollection, `FeatureCollection`>> should have at least one outgoing http://www.w3.org/2000/01/rdf-schema#member[rdfs:member] relation
...
| Violation
| An instance of <<class-featurecollection, `FeatureCollection`>> should only have outgoing http://www.w3.org/2000/01/rdf-schema#member[rdfs:member] going to <<class-feature, `Feature`>> instances
| <<req_core_feature-collection-class>>

|http://www.opengis.net/def/geosparql/validator/S22-GeometryCollectionClass-minOneMember-geometry-sub[Shape 22a]
| Violation
| An instance of <<class-geometrycollection, `GeometryCollection`>> should have at least one outgoing http://www.w3.org/2000/01/rdf-schema#member[rdfs:member] relation
...
| Violation
| An instance of <<class-geometrycollection, `GeometryCollection`>> should only have outgoing http://www.w3.org/2000/01/rdf-schema#member[rdfs:member] relations to <<class-geometry, `Geometry`>> instances
| <<req_core_geometry-collection-class>>

|http://www.opengis.net/def/geosparql/validator/S23-SpatialObjectCollection-minOneMember-spatialObject-sub[Shape 23a]
| Violation
| An instance of <<class-spatialobjectcollection, `SpatialObjectCollection`>> should have at least one outgoing http://www.w3.org/2000/01/rdf-schema#member[rdfs:member] relation
...
| Violation
| An instance of <<class-spatialobjectcollection, `SpatialObjectCollection`>> should only have outgoing http://www.w3.org/2000/01/rdf-schema#member[rdfs:member] relations going to <<class-spatialobject, `SpatialObject`>> instances, or subclasses of them
| <<req_core_spatial-object-collection-class>>
|===
