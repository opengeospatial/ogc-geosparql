...
This section establishes the requirements for representing Geometry data in RDF based on Well-Known Binary (WKB) as defined by _Simple Features Access_ <<OGCSFACA>> <<ISO19125-1>>. It defines the RDFS Datatype: <<RDFS Datatype: geo:ewkbLiteral, WKB Literal>> and one property, <<Property: geo:asWKB, as WKB>>. The literal definition in addition, allows for 3D types present in the Extended Well-Known Binary specification.

===== RDFS Datatypes: geo:wkbLiteral

The datatype http://www.opengis.net/ont/geosparql#ewkbLiteral[`geo:ewkbLiteral`] is used to contain the Extended Well-Known Binary (WKB) serialization of a Geometry.
...
|===
| *Req {counter:req}* All RDFS Literals of type <<RDFS Datatype: geo:ewkbLiteral, `geo:ewkbLiteral`>> shall consist of an optional IRI identifying the coordinate reference system and a required Well-Known Binary (WKB) description of a geometric value encoded in Base64. Valid <<RDFS Datatype: geo:ewkbLiteral, `geo:ewkbLiteral`>> instances are formed by a WKB string as defined in <<ISO13249>> or by concatenating a valid absolute IRI, as defined in <<IETF3987>>, enclosed in angled brackets (`<` & `>`) followed by whitespace as a separator and a WKB string as defined in <<ISO13249>>. The Well-Known Binary description shall allow the dimensionality flags defined in the Extended Well-Known Binary specification to represent 3D geometries.
...
The token `opt-iri-and-whitespace` may be either an IRI and whitespace (spaces, tabs, newlines) or nothing (`""`), the token `IRI` (Internationalized Resource Identifier) is essentially a web address and is defined in <<IETF3987>> and the token `LWSP`, a binary representation in WKB serialized as Base64. `geometry-data` is the Well-Known Binary representation of the Geometry, defined in <<ISO13249>>.

In the absence of a leading spatial reference system IRI, the following spatial reference system IRI will be assumed: http://www.opengis.net/def/crs/OGC/1.3/CRS84[`+<http://www.opengis.net/def/crs/OGC/1.3/CRS84>+`]. This IRI denotes WGS 84 longitude-latitude.

[#req_geometry_extension_wkb-literal-default-srs]
|===
| *Req {counter:req}* The IRI http://www.opengis.net/def/crs/OGC/1.3/CRS84[`+<http://www.opengis.net/def/crs/OGC/1.3/CRS84>+`] shall be assumed as the spatial reference system for <<RDFS Datatype: geo:wkbLiteral, `geo:ewkbLiteral`>> instances that do not specify an explicit spatial reference system IRI. IRIs defined as SRIDs in an Extended Well-Known Binary format shall be ignored.
...
|===
| *Req {counter:req}* Coordinate tuples within <<RDFS Datatype: geo:ewkbLiteral, `geo:ewkbLiteral`>> shall be interpreted using the axis order defined in the spatial reference system used.
...
The example <<RDFS Datatype: geo:ewkbLiteral, WKB Literal>> below encodes a point Geometry using the default WGS84 geodetic longitude-latitude spatial reference system:
...
|===
| *Req 18* An empty RDFS Literal of type <<RDFS Datatype: geo:ewkbLiteral, `geo:ewkbLiteral`>> shall be interpreted as an empty Geometry.
...
```
geof:asWKB (geom: ogc:geomLiteral): geo:ewkbLiteral
```
...
.Geocode Literal SRS
====
RDFS Literals of type <<RDFS Datatype: geo:geocodeLiteral, `geo:geocodeLiteral`>> contain a Geocoding system definition in the form of an IRI. The SRS for the All literals of this type shall according to the KML specification only be encoded in and assumed to use the WGS84 geodetic longitude-latitude spatial reference system (http://www.opengis.net/def/crs/OGC/1.3/CRS84[`http://www.opengis.net/def/crs/OGC/1.3/CRS84`]).
====
...
<<Function: geof:latitude, `geof:latitude`>>, 
<<Function: geof:longitude, `geof:longitude`>>, 
...
The function http://www.opengis.net/def/function/geosparql/boundingPolygons[`geof:boundingPolygons`] returns a collection of polygons which bound the nth polgon of a given PolyhedralSurface `geom`.
...
[source,turtle]
----
geof:latitude (geom: ogc:geomLiteral): xsd:double
----

The function http://www.opengis.net/def/function/geosparql/latitude[`geof:longitude`] returns the latitude of `geom` in meters. If the geometry is a point it returns the point's latitude. If the geometry is a more complex geometry it returns the latitude of its centroid as calculated by geof:centroid.

[%unnumbered]
[source,turtle]
----
geof:longitude (geom: ogc:geomLiteral): xsd:double
----

The function http://www.opengis.net/def/function/geosparql/longitude[`geof:longitude`] returns the latitude of `geom` in meters. If the geometry is a point it returns the point's latitude. If the geometry is a more complex geometry it returns the latitude of its centroid as calculated by geof:centroid.
