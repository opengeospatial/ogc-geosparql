...
The function http://www.opengis.net/def/function/geosparql/latitude[`geof:latitude`] returns the latitude of `geom` in the unit that is proposed by the coordinate reference system associated with the geometry. This function is defined only for such `geom` representing Points.
