...
Membership of the general <<Class: geo:SpatialObjectCollection, Spatial Object Collection>> that defines this class is restricted to instances of <<Class: geo:Geometry, Geometry>>. `geo:GeometryCollection` members are to be indicated with the http://www.w3.org/2000/01/rdf-schema#member[`rdfs:member`] property.
...
This section establishes the requirements for representing Geometry data in RDF based on Well-Known Binary (WKB) as defined by _Simple Features Access_ <<OGC06-103r4>> <<ISO19125-1>>. It defines the RDFS Datatype: <<RDFS Datatypes: geo:wkbLiteral, WKB Literal>> and one property, <<Property: geo:asWKB, as WKB>>. The literal definition in addition, allows for 3D types present in the Extended Well-Known Binary specification.

===== RDFS Datatypes: geo:wkbLiteral

The datatype http://www.opengis.net/ont/geosparql#wkbLiteral[`geo:ewkbLiteral`] is used to contain the Extended Well-Known Binary (WKB) serialization of a Geometry.
...
|===
| *Req {counter:req}* All RDFS Literals of type <<RDFS Datatypes: geo:wkbLiteral, `geo:wkbLiteral`>> shall consist of an optional IRI identifying the coordinate reference system and a required Well-Known Binary (WKB) description of a geometric value encoded in Base64. Valid <<RDFS Datatypes: geo:wkbLiteral, `geo:wkbLiteral`>> instances are formed by a WKB string as defined in <<ISO13249>> or by concatenating a valid absolute IRI, as defined in <<IETF3987>>, enclosed in angled brackets (`<` & `>`) followed by whitespace as a separator and a WKB string as defined in <<ISO13249>>. The Well-Known Binary description shall allow the dimensionality flags defined in the Extended Well-Known Binary specification to represent 3D geometries.
...
|===
| *Req {counter:req}* The IRI http://www.opengis.net/def/crs/OGC/1.3/CRS84[`+<http://www.opengis.net/def/crs/OGC/1.3/CRS84>+`] shall be assumed as the spatial reference system for <<RDFS Datatypes: geo:wkbLiteral, `geo:wkbLiteral`>> instances that do not specify an explicit spatial reference system IRI. IRIs defined as SRIDs in an Extended Well-Known Binary format shall be ignored.
...
|===
| *Req {counter:req}* Coordinate tuples within <<RDFS Datatypes: geo:wkbLiteral, `geo:wkbLiteral`>> shall be interpreted using the axis order defined in the spatial reference system used.
...
The example <<RDFS Datatypes: geo:wkbLiteral, WKB Literal>> below encodes a point Geometry using the default WGS84 geodetic longitude-latitude spatial reference system:
...
|===
| *Req 18* An empty RDFS Literal of type <<RDFS Datatypes: geo:wkbLiteral, `geo:ewkbLiteral`>> shall be interpreted as an empty Geometry.
