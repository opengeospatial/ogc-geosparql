...
Some of the properties defined in these vocabularies, such as DCAT2's https://www.w3.org/TR/vocab-dcat-2/#Property:dataset_spatial_resolution[`dcat:spatialResolution`] have motivated the inclusion of new properties in this version of GeoSPARQL. In this case the equivalent property is <<prop-hasspatialresolution, `hasSpatialResolution`>>. The GeoSPARQL 1.1 Standards Working Group charter <<CHARTER>> contains references to a number of vocabularies/ontologies that were influential in the generation of this version of GeoSPARQL.
...
A single root geometry class is defined: <<class-geometry, `Geometry`>>. In addition, properties are defined for describing geometry data and for associating geometries with features.

One container class is defined: <<class-geometrycollection, `GeometryCollection`>>. 

[[class-geometry]]
==== Geometry

The class http://www.opengis.net/ont/geosparql#Geometry[`Geometry`] is conceptually derived from UML class `Geometry` in <<ISO19107>> which is that standard's "root class of the geometric object taxonomy and supports interfaces common to all geographically referenced geometric objects". `Geometry` is defined by the following:
...
.Geometry Class
====
Implementations shall allow the RDFS class <<class-geometry, `Geometry`>> to be used in SPARQL graph patterns.
====

[[class-geometrycollection]]
==== Geometry Collection

The class <<class-geometrycollection, `GeometryCollection`>> is defined by the following:
...
Membership of the general <<class-spatialobjectcollection, `SpatialObjectCollection`>> that defines this class is restricted to instances of <<class-geometry, `Geometry`>>. `GeometryCollection` members are to be indicated with the http://www.w3.org/2000/01/rdf-schema#member[`rdfs:member`] property.

[NOTE]
====
There is no RDF/ontology relationship between this <<class-geometrycollection, `GeometryCollection`>> class and the Simple Features Vocabulary's http://www.opengis.net/ont/sf#GeometryCollection[`sf:GeometryCollection`] class since the former is a collection of `Geometry` objects and the latter is to be used for compound geometry literals.

http://www.opengis.net/ont/sf#GeometryCollection[`sf:GeometryCollection`] instances can act as input or output of GeoSPARQL functions whereas <<class-geometrycollection, `GeometryCollection`>> instances are more likely to be used for grouping `Geometry` objects for other purposes.
...
As per the expected use of http://www.opengis.net/ont/sf#GeometryCollection[`sf:GeometryCollection`] instances mentioned above: the uses of multi-geometry literals and <<class-geometrycollection, `GeometryCollection`>> instances is expected to be different too.
====
...
====
Implementations shall allow the RDFS class <<class-geometrycollection, `GeometryCollection`>> to be used in SPARQL graph patterns.
====
...
The property <<prop-dimension, `dimension`>> is used to link a Geometry object to its topological dimension, which must be less than or equal to the coordinate dimension. In non-homogeneous collections, this will return the largest topological dimension of the contained objects.
...
[[prop-coordinatedimension]]
==== coordinateDimension

The property <<prop-coordinatedimension, `coordinateDimension`>> is defined to link a Geometry object to the dimension of direct positions (coordinate tuples) used in the Geometry's definition.
...
[[prop-hasspatialdimension]]
==== hasSpatialDimension

The property <<prop-hasspatialdimension, `hasSpatialDimension`>> is defined to link a Geometry object to the dimension of the spatial portion of the direct positions (coordinate tuples) used in its serializations. If the direct positions do not carry a measure coordinate, this will be equal to the coordinate dimension.
...
[[prop-hasspatialresolution]]
==== hasSpatialResolution

The property <<prop-hasspatialresolution, `hasSpatialResolution`>> is defined to indicate the spatial resolution of the elements within a Geometry. Spatial resolution specifies the level of detail of a Geometry. It is the smallest distinguishable distance between adjacent coordinate sets. This property is not applicable to a point Geometry, because a point consists of a single coordinate set.

Since this property is defined for a <<class-geometry, `Geometry`>>, all literal representations of that Geometry instance must have the same spatial resolution.
...
[[prop-hasmetricspatialresolution]]
==== hasMetricSpatialResolution

The property <<prop-hasmetricspatialresolution, `hasMetricSpatialResolution`>> is similar to <<prop-hasspatialresolution, `hasSpatialResolution`>>, except that the unit of resolution is always meter (the standard distance unit of the International System of Units).
...
[[prop-hasmetricspatialaccuracy]]
==== hasMetricSpatialAccuracy

The property <<prop-hasmetricspatialaccuracy, `hasMetricSpatialAccuracy`>> is similar to <<prop-hasspatialaccuracy, has spatial accuracy>>, but is easier to specify and use because the unit of distance is always meter (the standard distance unit of the International System of Units).
...
The property <<prop-isempty, `isEmpty`>> will indicate a Boolean object set to `true` if and only if the Geometry contains no information.
...
The property <<prop-issimple, `isSimple`>> will indicate a Boolean object set to `true` if and only if the Geometry contains no self-intersections, with the possible exception of its boundary.
...
[[prop-hasserialization]]
==== hasSerialization

The property <<prop-hasserialization, `hasSerialization`>> is defined to connect a Geometry with its text-based serialization (e.g., its WKT serialization).
...
This section establishes the requirements for representing Geometry data in RDF based on Well-Known Text (WKT) as defined by _Simple Features Access_ <<OGCSFACA>> <<ISO19125-1>>. It defines one RDFS Datatype: <<datatype-wktliteral, WKT Literal>> and one property, <<func-aswkt, as WKT>>.

[[datatype-wktliteral]]
===== wktLiteral

The datatype <<datatype-wktliteral, `wktLiteral`>> is used to contain the Well-Known Text (WKT) serialization of a Geometry.
...
.WKT Literal
====
All RDFS Literals of type <<datatype-wktliteral, `wktLiteral`>> shall consist of an optional IRI identifying the coordinate reference system and a required Well Known Text (WKT) description of a geometric value. Valid <<datatype-wktliteral, `wktLiteral`>> instances are formed by either a WKT string as defined in <<ISO13249>> or by concatenating a valid absolute IRI, as defined in <<IETF3987>>, enclosed in angled brackets (`<` and `>`) followed by whitespace as a separator, and a WKT string as defined in <<ISO13249>>.
====
...
====
The IRI http://www.opengis.net/def/crs/OGC/1.3/CRS84[`+<http://www.opengis.net/def/crs/OGC/1.3/CRS84>+`] shall be assumed as the spatial reference system for <<datatype-wktliteral, `wktLiteral`>> instances that do not specify an explicit spatial reference system IRI.
====
...
====
Coordinate tuples within <<datatype-wktliteral, `wktLiteral`>> shall be interpreted using the axis order defined in the spatial reference system used.
====

The example <<datatype-wktliteral, WKT Literal>> below encodes a point Geometry using the default WGS84 geodetic longitude-latitude spatial reference system:
...
.Empty WKT Literal
====
An empty RDFS Literal of type <<datatype-wktliteral, `wktLiteral`>> shall be interpreted as an empty Geometry.
====
...
The property <<prop-aswkt, `asWKT`>> is defined to link a Geometry with its WKT serialization.

[[req_geometry_extension_geometry-as-wkt-literal]]
[requirement,identifier="/req/geometry-extension/geometry-as-wkt-literal"]
.asWKT Property
====
Implementations shall allow the RDF property <<prop-aswkt, `asWKT`>> to be used in SPARQL graph patterns.
====
...
.asWKT Function
====
Implementations shall support <<func-aswkt, `asWKT`>> as a SPARQL extension function.
====
...
<<datatype-gmlliteral, GML Literal>> and one property, <<func-asgml, as GML>>.

[[datatype-gmlliteral]]
===== gmlLiteral

The datatype <<datatype-gmlliteral, gmlLiteral>> is used to contain the Geography Markup Language (GML) serialization of a Geometry.
...
.Empty GML Literal
====
An empty <<datatype-gmlliteral, `gmlLiteral`>> shall be interpreted as an empty Geometry.
====
...
The property <<prop-asgml, `asGML`>> is defined to link a Geometry with its GML serialization.

[[req_geometry_extension_geometry-as-gml-literal]]
[requirement,identifier="/req/geometry-extension/geometry-as-gml-literal"]
.asGML Property
====
Implementations shall allow the RDF property <<prop-asgml, `asGML`>> to be used in SPARQL graph patterns.
====
...
.asGML Function
====
Implementations shall support <<func-asgml, `asGML`>> as a SPARQL extension function.
====

==== GeoJSON

This section establishes a Requirements class for representing Geometry data in RDF based on Geographic JavaScript Object Notation (GeoJSON) as defined by <<GeoJSON>>. It defines one RDFS Datatype: <<datatype-geojsonliteral, geoJsonLiteral>> and one property, <<func-asgeojson, as GeoJSON>>.

[[datatype-geojsonliteral]]
===== geoJSONLiteral

The datatype <<datatype-geojsonliteral, geoJsonLiteral>> is used to contain the GeoJSON serialization of a Geometry.
...
Valid <<datatype-geojsonliteral, GeoJSON Literal>> instances are formed by encoding Geometry information as a Geometry object as defined in the GeoJSON specification <<GEOJSON>>.

[[req_geometry_extension_geojson-literal]]
[requirement,identifier="/req/geometry-extension/geojson-literal"]
.GeoJSON Literal
====
All <<datatype-geojsonliteral, `geoJSONLiteral`>> instances shall consist of the Geometry objects as defined in the GeoJSON specification <<GEOJSON>>.
====
...
.GeoJSON Literal SRS
====
RDFS Literals of type <<datatype-geojsonliteral, `geoJSONLiteral`>> do not contain a SRS definition. All literals of this type shall, according to the GeoJSON specification, be encoded only in, and be assumed to use, the WGS84 geodetic longitude-latitude spatial reference system (http://www.opengis.net/def/crs/OGC/1.3/CRS84[`http://www.opengis.net/def/crs/OGC/1.3/CRS84`]).
====

The example <<datatype-geojsonliteral, GeoJSON Literal>> below encodes a point Geometry using the default WGS84 geodetic longitude-latitude spatial reference system for Simple Features 1.0:
...
====
An empty RDFS Literal of type <<datatype-geojsonliteral, `geoJSONLiteral`>> shall be interpreted as an empty Geometry, i.e. `{"geometry": null}` in GeoJSON .
====

[[prop-asgeojson]]
===== asGeoJSON

The property <<prop-asgeojson, `asGeoJSON`>> is defined to link a Geometry with its GeoJSON serialization.

[[req_geometry_extension-as-geojson-literal]]
[requirement,identifier="/req/geometry-extension/geometry-as-geojson-literal"]
.asGeoJSON Property
====
Implementations shall allow the RDF property <<prop-asgeojson, `asGeoJSON`>> to be used in SPARQL graph patterns.
====
...
.asGeoJSON Function
====
Implementations shall support <<func-asgeojson, `asGeoJSON`>> as a SPARQL extension function.
====
...
<<datatype-kmlliteral, KML Literal>> and one property, <<func-askml, asKML>>.

[[datatype-kmlliteral]]
===== kmlLiteral

The datatype <<datatype-kmlliteral, kmlLiteral>> is used to contain the Keyhole Markup Language (KML) serialization of a Geometry.
...
Valid <<datatype-kmlliteral, kmlLiteral>> instances are formed by encoding Geometry information as a Geometry object as defined in the KML specification <<OGCKML>>.
...
.KML Literal
====
All <<datatype-kmlliteral, `kmlLiteral`>> instances shall consist of the Geometry objects as defined in the KML specification <<OGCKML>>.
====
...
.KML Literal SRS
====
RDFS Literals of type <<datatype-kmlliteral, `kmlLiteral`>> do not contain a SRS definition. All literals of this type shall according to the KML specification only be encoded in and assumed to use the WGS84 geodetic longitude-latitude spatial reference system (http://www.opengis.net/def/crs/OGC/1.3/CRS84[`http://www.opengis.net/def/crs/OGC/1.3/CRS84`]).
====

The example <<datatype-kmlliteral, KML Literal>> below encodes a point Geometry using the default WGS84 geodetic longitude-latitude spatial reference system for Simple Features 1.0:
...
.Empty KML Literal
====
An empty RDFS Literal of type <<datatype-kmlliteral, `kmlLiteral`>> shall be interpreted as an empty Geometry .
====

[[prop-askml]]
===== asKML

The property <<prop-askml, `asKML`>> is defined to link a Geometry with its KML serialization.

[[req_geometry_extension_geometry-as-kml-literal]]
[requirement,identifier="/req/geometry-extension/geometry-as-kml-literal"]
.asKML Property
====
Implementations shall allow the RDF property <<prop-askml, `asKML`>> to be used in SPARQL graph patterns.
====

The property <<prop-askml, as KML>> is used to link a geometric element with its KML serialization.
...
.asKML Function
====
Implementations shall support <<func-askml, `asKML`>> as a SPARQL extension function.
====
...
[[datatype-dggsliteral]]
===== dggsLiteral

The datatype <<datatype-dggsliteral, dggsLiteral>> is used to contain the Discrete Global Grid System (DGGS) serialization of a Geometry.
...
Valid <<datatype-dggsliteral, dggsLiteral>> instances are formed by encoding Geometry information according to a specific DGGS implementation. The specific implementation should be indicated by use of a subclass of the <<datatype-dggsliteral, dggsLiteral>> datatype.

[[req_geometry_extension_dggs-literal]]
[requirement,identifier="/req/geometry-extension-dggs/dggs-literal"]
.DGGS Literal
====
All RDFS Literals of type <<datatype-dggsliteral, `dggsLiteral`>> shall consist of an IRI identifying the specific DGGS and a representation of the DGGS geometry data. The IRI shall be enclosed in angled brackets (`<` and `>`) followed by whitespace as a separator, and then the DGGS geometry data, formulated according to the identified DGGS.
====
...
.Empty DGGS Literal
====
An empty RDFS Literal of type <<datatype-dggsliteral, `dggsLiteral`>>, shall be interpreted as an empty `Geometry`.
====
...
The property <<prop-asdggs, `asDGGS`>> is defined to link a Geometry with its DGGS serialization.
...
.asDGGS Function
====
Implementations shall support <<func-asdggs, `asDGGS`>>  as a SPARQL extension function.
====
...
<<func-boundary, `boundary`>>
<<func-boundingcircle, `boundingCircle`>>,
<<func-metricbuffer, `metricBuffer`>>,
<<func-buffer, `buffer`>>,
<<func-centroid, `centroid`>>,
<<func-convexhull, `convexHull`>>,
<<func-concavehull, `concaveHull`>>,
<<func-coordinatedimension, `coordinateDimension`>>,
<<func-difference, `difference`>>,
<<func-dimension, `dimension`>>,
<<func-metricdistance, `metricDistance`>>,
<<func-distance, `distance`>>,
<<func-envelope, `envelope`>>,
<<func-geometrytype, `geometryType`>>,
<<func-intersection, `intersection`>>,
<<func-is3d, `is3D`>>,
<<func-isempty, `isEmpty`>>,
<<func-ismeasured, `isMeasured`>>,
<<func-issimple, `isSimple`>>,
<<func-hasspatialdimension, `hasSpatialDimension`>>,
<<func-symdifference, `symDifference`>>,
<<func-transform, `transform`>> and
<<func-union, `union`>>
...
====
Implementations shall support the functions 
<<func-metriclength, `metricLength`>>,
<<func-length, `length`>>,
<<func-metricperimeter, `metricPerimeter`>>,
<<func-perimeter, `perimeter`>>,
<<func-metricarea, `metricArea`>>,
<<func-area, `area`>>,
<<func-geometryn, `geometryN`>>,
<<func-maxx, `maxX`>>,
<<func-maxy, `maxY`>>,
<<func-maxz, `maxZ`>>,
<<func-minx, `minX`>>,
<<func-miny, `minY`>>,
<<func-minz, `minZ`>> and
<<func-numgeometries, `numGeometries`>>
...
====
Implementations shall support the functions of <<req_geometry_extension_query-functions>> for DGGS geometry literals
...
====
Implementations shall support the functions of <<req_geometry_extension_query-functions-non-sf>> for DGGS geometry literals
as SPARQL extension functions which are defined in this standard, for non-DGGS geometry literals.
====
...
This extension mechanism enables GeoSPARQL implementations to simultaneously support multiple geometry serializations. For example, a system that supports <<datatype-wktliteral, WKT Literal>> serializations may also support <<datatype-gmlliteral, GML Literal>> serializations and consequently would not raise an error if it encounters multiple geometry datatypes while processing a given query.

[NOTE,keep-separate=true]
====
Several non-topological query functions use a unit of measure IRI. See the <<uom>>. Also, the OGC has recommended units of measure vocabularies for use, see the OGC Definitions Serverfootnote:[https://www.ogc.org/def-server].
====
...
The function http://www.opengis.net/def/function/geosparql/metricArea[`geof:metricArea`] returns the area of `geom` in square meters. Must return zero for all geometry types other than Polygon. This function is similar to <<func-area, `area`>> but does not need a specification of measurement unit.
...
The function http://www.opengis.net/def/function/geosparql/area[`geof:area`] returns the area of `geom`. Must return zero for all geometry types other than Polygon. This function is similar to <<func-metricarea, `metricArea`>>, which does not need a specification of measurement unit.
...
The function http://www.opengis.net/def/function/geosparql/buffer[`geof:buffer`] returns a geometric object that represents all Points whose distance from `geom` is less than or equal to the `radius` measured in `units`. Calculations are in the spatial reference system of `geom`. This function is similar to <<func-metricbuffer, `metricBuffer`>>, which does not need a specification of measurement unit.
...
The function http://www.opengis.net/def/function/geosparql/is3D[`geof:is3D`] Returns true if `geom` has z coordinate values. See <<GeoSPARQL and Simple Features (SFA-CA)>> for a description of GeoSPARQL's limitations: currently it supports 2.5D and geometric functions ignore Z values in calculations.
...
The function http://www.opengis.net/def/function/geosparql/isMeasured[`geof:isMeasured`] returns true if `geom` has m coordinate values, as used e.g. in linear referencing. See <<GeoSPARQL and Simple Features (SFA-CA)>> for a description of GeoSPARQL's limitations: at the moment it does not support linear referencing and M values are ignored.
...
The function http://www.opengis.net/def/function/geosparql/length[`geof:length`] returns the length of `geom`. The longest length from any one dimension is returned. This function is similar to <<func-metriclength, `metricLength`>>, which does not need a specification of measurement unit.
...
The function http://www.opengis.net/def/function/geosparql/perimeter[`geof:perimeter`] returns the perimeter of  `geom` in the unit specified by the unit parameter for areal geometries. For non-areal geometries the result is equivalent to <<func-length, `length`>>.

[[func-metricperimeter]]
==== metricPerimeter
...
The function http://www.opengis.net/def/function/geosparql/metricPerimeter[`geof:metricPerimeter`] returns the perimeter of `geom`. It is similar to the function <<func-perimeter, `perimeter`>>, but always returns the result in meters.

[[func-hasspatialdimension]]
==== hasSpatialDimension
...
The function http://www.opengis.net/def/function/geosparql/hasSpatialDimension[`geof:hasSpatialDimension`] returns the spatial dimension of `geom`.
...
.SRID Function
====
Implementations shall support <<func-getsrid, `getSRID`>> as a SPARQL extension function.
====
...
The function <<func-getsrid, `getSRID`>> returns the spatial reference system IRI for `geom`.
...
<<func-aggboundingbox, `aggBoundingBox`>>,
<<func-aggboundingcircle, `aggBoundingCircle`>>,
<<func-aggcentroid, `aggCentroid`>>,
<<func-aggconcavehull, `aggConcaveHull`>>,
<<func-aggconvexhull, `aggConvexHull`>> and
<<func-aggunion, `aggUnion`>>
...
The function <<func-aggboundingbox, `aggBoundingBox`>> calculates a minimum bounding box - rectangle - of the set of given geometries.

[NOTE,keep-separate=true]
====
This function is similar in nature to <<func-envelope, `envelope`>> used to calculate the bounding box of just one geometry.
====

[[func-aggboundingcircle]]
==== aggBoundingCircle
[%unnumbered]
[source,sparql]
----
...
The function <<func-aggboundingcircle, `aggBoundingCircle`>> calculates a minimum bounding circle of the set of given geometries.

[NOTE,keep-separate=true]
====
This function is similar in name to <<func-boundingcircle, `boundingCircle`>> used to calculate the bounding circle of just one geometry.
====
...
The function <<func-aggcentroid, `aggCentroid`>> calculates the centroid of the set of given geometries.

[NOTE,keep-separate=true]
====
This function is similar in name to <<func-centroid, `centroid`>> used to calculate the centroid of just one geometry.
====
...
[NOTE,keep-separate=true]
====
This function is similar in name to <<func-concavehull, `concaveHull`>> used to calculate the concave hull of just one geometry.
====
...
[NOTE,keep-separate=true]
====
This function is similar in name to <<func-convexhull, `convexHull`>> used to calculate the convex hull of just one geometry.
====
...
NOTE: This function is similar in name to <<func-union, `union`>> used to calculate the union of just two geometries.
