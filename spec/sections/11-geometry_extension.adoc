...
|===
| *Req {counter:req}* An empty RDFS Literal of type <<RDFS Datatypes: geo:ewkbLiteral, `geo:ewkbLiteral`>> shall be interpreted as an empty Geometry.
...
[[req_geometry_extension_geometry-as-ply-literal]]
...
This section establishes the requirements for representing Geometry data in RDF based on the Graphics Library Transmission Format (GLTF). 
...
|===
| *Req {counter:req}* An empty RDFS Literal of type <<RDFS Datatypes: geo:gltfLiteral, `geo:gltfLiteral`>> shall be interpreted as an empty Geometry.
...
.X3D Literal
====
All <<RDFS Datatype: geo:x3dLiteral, `geo:x3dLiteral`>> instances shall consist of a valid element as defined in the X3D standard.
====
...
The property http://www.opengis.net/ont/geosparql#asX3D[`geo:asX3D`] is defined as a link between a Geometry and its GML serialization.
...
.OBJ Literal
====
All <<RDFS Datatype: geo:objLiteral, `geo:objLiteral`>> instances shall consist of the Geometry objects defined in the OBJ specification.
====
