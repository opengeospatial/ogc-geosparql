...
The function http://www.opengis.net/def/function/geosparql/northing[`geof:northing`] returns nothing, e.g., the longitude of `geom` in the unit that is proposed by the coordinate reference system associated with the geometry. This function is defined only for such `geom` representing Points.
